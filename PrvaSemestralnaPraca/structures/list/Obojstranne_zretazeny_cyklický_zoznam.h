...
namespace structures {
	...
	inline int ObojZretLinkedList<T>::getIndexOf(const T& data)
	{
		...
		if (result != nullptr) // ak mam nieco v zozname
		{
			do {
			...
			} while (result != first_); // nullptr nemam inokedy lebo mam smernik na first_ a na last_
		}
		return -1;
	}
	...
	inline ObojZretLinkedListItem<T>* ObojZretLinkedList<T>::getItemAtIndex(int index) const
	{
		...
		if (index < static_cast<int>(size_) / 2) // prva polka zoznamu
		{
			ObojZretLinkedListItem<T>* pomItem = first_;
			for (int i = 0; i < index; i++)
			{
				pomItem = pomItem->getNext();
			}
			return pomItem;
		}
		else // druha polka zoznamu
		{
			ObojZretLinkedListItem<T>* pomItem = last_;
			for (size_t i = 0; i < size_ - 1 - index; i++)
			{
				pomItem = pomItem->getPrevious();
			}
			return pomItem;
		}
	}
